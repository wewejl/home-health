...
struct LoginView: View {
    @StateObject private var smsViewModel = LoginViewModel()
    @StateObject private var passwordViewModel = PasswordLoginViewModel()
    @FocusState private var focusedField: LoginField?
    @Environment(\.openURL) private var openURL
    
    @State private var loginMethod: LoginMethod = .sms
    ...
    var body: some View {
            ...
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 0) {
                        ...
                        // 登录方式切换
                        LoginMethodToggle(selectedMethod: $loginMethod)
                            .padding(.horizontal, horizontalPadding)
                            .padding(.bottom, AdaptiveSpacing.item)
                        ...
                        if loginMethod == .sms {
                            ...
                            PasswordLoginFormCard(
                                viewModel: passwordViewModel,
                                focusedField: $focusedField,
                                openURL: openURL,
                                availableWidth: safeWidth - horizontalPadding * 2,
                                onRegister: { showRegisterView = true },
                                onForgotPassword: { showResetPasswordView = true }
                            )
                            .padding(.horizontal, horizontalPadding)
                        }
                ...
                if smsViewModel.isLoading || passwordViewModel.isLoading {
                    LoadingOverlay()
                }
            }
        ...
        .onDisappear {
            smsViewModel.cleanup()
            passwordViewModel.cleanup()
        }
        .onChangeCompat(of: smsViewModel.uiState) { newState in
            if newState == .success {
                onLoginSuccess?()
            }
        }
        .onChangeCompat(of: passwordViewModel.uiState) { newState in
            if newState == .success {
                onLoginSuccess?()
            }
        }
        .alert("提示", isPresented: $smsViewModel.showError) {
            Button("确定", role: .cancel) {}
        } message: {
            Text(smsViewModel.errorMessage)
        }
        .alert("提示", isPresented: $passwordViewModel.showError) {
            Button("确定", role: .cancel) {}
        } message: {
...
// MARK: - 登录方式切换
struct LoginMethodToggle: View {
    @Binding var selectedMethod: LoginMethod
    
    var body: some View {
        HStack(spacing: 0) {
            ForEach(LoginMethod.allCases, id: \.rawValue) { method in
                Button(action: {
                    ...
                    Text(method.rawValue)
                        .font(.system(size: AdaptiveFont.subheadline, weight: selectedMethod == method ? .semibold : .regular))
                        .foregroundColor(selectedMethod == method ? PremiumColorTheme.primaryColor : PremiumColorTheme.textSecondary)
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, ScaleFactor.padding(10))
                        .background(
                            selectedMethod == method ?
                            PremiumColorTheme.primaryColor.opacity(0.12) :
                            Color.clear
                        )
        ...
        .background(
            RoundedRectangle(cornerRadius: AdaptiveSize.cornerRadiusSmall, style: .continuous)
                .fill(Color.dynamicColor(
                    ...
                    dark: Color(red: 0.15, green: 0.15, blue: 0.18).opacity(0.8)
                ))
        )
    }
}
...
// MARK: - 密码登录表单卡片
struct PasswordLoginFormCard: View {
    ...
    var body: some View {
        GlassmorphicCard {
            VStack(spacing: itemSpacing) {
                ...
                // 手机号输入
                PasswordPhoneInputSection(viewModel: viewModel, focusedField: focusedField)
                ...
                // 登录按钮
                PrimaryButton(title: "登录", action: viewModel.loginWithPassword)
...
// MARK: - 密码手机号输入
struct PasswordPhoneInputSection: View {
    @ObservedObject var viewModel: PasswordLoginViewModel
    var focusedField: FocusState<LoginField?>.Binding
...
struct PasswordInputField: View {
    ...
    private var fontSize: CGFloat { AdaptiveFont.body }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: ScaleFactor.spacing(6)) {
            ...
            HStack(spacing: ScaleFactor.spacing(12)) {
                ...
                if let toggle = toggleVisibility {
    ...
    var body: some View {
        GlassmorphicCard {
            VStack(spacing: itemSpacing) {
                ...
                PrimaryButton(title: "登录", action: viewModel.login)
                    .padding(.top, ScaleFactor.padding(8))
...
enum AppURLConstants {
    ...
    static let helpCenterURL = "https://xinlinyisheng.com/help"
...
struct SocialLoginSection: View {
    ...
    var body: some View {
        VStack(spacing: AdaptiveSpacing.section) {
            HStack(spacing: ScaleFactor.spacing(12)) {
                Rectangle()
                    .fill(PremiumColorTheme.textTertiary.opacity(0.5))
                    .frame(height: 1)
                Text("其他登录方式")
                    .font(.system(size: AdaptiveFont.footnote, weight: .regular, design: .rounded))
                    .foregroundColor(PremiumColorTheme.textTertiary)
            ...
            HStack(spacing: AdaptiveSpacing.section + ScaleFactor.spacing(12)) {
                SocialLoginButton(icon: "apple.logo", color: .primary, label: "Apple") {}
                SocialLoginButton(icon: "message.fill", color: PremiumColorTheme.accentColor, label: "微信") {}
            }
        }
        .opacity(showContent ? 1 : 0)
...
struct SocialLoginButton: View {
    ...
    private var buttonSize: CGFloat { ScaleFactor.size(56) }
    private var iconSize: CGFloat { AdaptiveFont.title1 }
