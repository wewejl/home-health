...
from app.routes.sessions import migrate_legacy_state, router
...
class TestMigrateLegacyState:
    ...
    def test_json_string_state_parsing(self):
        """测试：JSON 字符串状态应正确解析"""
        import json
        legacy_state_str = '{"stage": "collecting", "chief_complaint": "头痛", "symptoms": ["发热"]}'
    ...
    def test_legacy_state_filters_unwanted_fields(self):
        """测试：旧版本状态中的不需要字段应被过滤"""
        legacy_state = {
            "questions_asked": 3,
            "session_id": "test-123",
...
class TestGetSessions:
    ...
    def test_get_sessions_empty_list(self, mock_db, mock_current_user):
        """测试：空会话列表应返回空数组"""
        # 创建正确的 mock 链
        ...
        from app.routes.sessions import get_sessions
        result = get_sessions(db=mock_db, current_user=mock_current_user)

        assert result == []

    def test_get_sessions_with_sessions(self, mock_db, mock_current_user, mock_sessions, mock_doctors):
        """测试：返回会话列表"""
        ...
        # SessionModel 在 routes/sessions.py 中是 Session 的别名
    ...
    def test_get_sessions_response_format(self, mock_db, mock_current_user, mock_sessions, mock_doctors):
        """测试：响应格式正确"""
        # 创建一个完整的 sessions query mock chain
        mock_filtered = Mock()
        mock_filtered.order_by.return_value.all.return_value = mock_sessions
...
class TestGetSessionMessages:
    """测试 GET /sessions/{session_id}/messages 端点"""
    ...
    def test_get_messages_session_not_found(self, mock_db, mock_current_user):
        """测试：会话不存在应返回 404"""
        ...
        from app.routes.sessions import get_session_messages, HTTPException
    ...
    def test_get_messages_with_messages(self, mock_db, mock_current_user, mock_session, mock_messages):
        """测试：返回消息列表"""
        ...
        from app.routes.sessions import get_session_messages, MessageListResponse
    ...
    def test_get_messages_with_pagination(self, mock_db, mock_current_user, mock_session):
        """测试：分页功能"""
        # 创建 25 条消息，带有正确的属性类型
        messages = []
        for i in range(1, 26):
        ...
        from app.routes.sessions import get_session_messages
    ...
    def test_get_messages_with_before_parameter(self, mock_db, mock_current_user, mock_session):
        """测试：before 参数用于分页"""
...
class TestStateCompatibility:
    ...
    def test_legacy_empty_state_compatibility(self):
        """测试：旧版本空 agent_state 兼容"""
        legacy_state = None
        result = migrate_legacy_state(legacy_state)
        assert result == {}

    def test_legacy_minimal_state_compatibility(self):
        """测试：旧版本最小状态兼容"""
        legacy_state = {
            "stage": "greeting",
            "chief_complaint": ""
    ...
    def test_new_empty_state_still_works(self):
        """测试：新版本空状态仍然可以工作"""
        # 新版本使用空状态作为初始状态
        new_state = {}
        result = migrate_legacy_state(new_state)
        assert result == {}
    ...
    def test_state_roundtrip(self):
        ...
        result = migrate_legacy_state(original)
