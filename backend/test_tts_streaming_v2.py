...
class BufferedStreamPlayer:
    ...
    def __init__(self, sample_rate=24000, channels=1, buffer_size=10):
    ...
    def write(self, audio_data: bytes):
        ...
        with self.lock:
            ...
            # 显示缓冲区状态
            buffer_ms = (sum(len(d) for d in self.queue) / 2 / self.sample_rate) * 1000
            print(f"[Player] 缓冲: {len(self.queue)} 块, {buffer_ms:.0f}ms")
...
async def test_streaming_tts_v2():
    """测试流式 TTS - 边接收边播放 (改进版)"""

    print("="*60)
    print("流式 TTS 测试 v2 - 带缓冲的边接收边播放")
    print("="*60)
    ...
    # 创建播放器
    player = BufferedStreamPlayer(sample_rate=24000, channels=1)
    ...
    try:
        async with websockets.connect(url, additional_headers=headers) as ws:
            ...
            # 等待播放完成
            print(f"\n[5/4] 等待播放完成...")
            estimated_duration = player.total_written / 2 / 24000  # 秒
...
if __name__ == "__main__":
    asyncio.run(test_streaming_tts_v2())
